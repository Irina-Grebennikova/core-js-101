...
function timeSpanToString(startDate, endDate) {
  ...
  let min = Math.floor(timespan / 60000);
  if (String(min).length < 2) min = `0${min}`;
  timespan -= 60000 * min;

  let sec = Math.floor(timespan / 1000);
  if (String(sec).length < 2) sec = `0${sec}`;

  let ms = timespan - 1000 * sec;
...
function angleBetweenClockHands(date) {
  ...
  const fraction = 360 / angle / 2;

  if (fraction === 0 || angle === 360 || angle === 0) return 0;

  return (Math.PI / fraction).toFixed(16);
}
